...
public class AddStudent extends JFrame implements ActionListener {
    ...
    AddStudent() {
        ...
        String[] course = {"B.Tech", "BCA", "BBA", "B.Sc", "B.Comm", "BA"};
        cb = new JComboBox<>(course);
        cb.setBounds(930, 100, 170, 30);
        cb.setBackground(Color.WHITE);
        add(cb);

        // Branch label and combo box
        JLabel lblbr = new JLabel("Branch :");
        lblbr.setBounds(830, 200, 170, 30);
        lblbr.setFont(new Font("serif", Font.BOLD, 20));
        add(lblbr);

        String[] branch = {"CSE", "ECE", "IT", "ME", "ASE", "AI/ML"};
        cbb = new JComboBox<>(branch);
        cbb.setBounds(930, 200, 170, 30);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == sub) {  // If submit button is clicked
            ...
            try {
                ...
                String query = "insert into stdus values('" + name + "','" + fname + "','" + roll + "','" + dob + "','" + add + "','" + ph + "','" + email + "','" + x + "','" + xii + "','" + adhar + "','" + c + "','" + br + "')";
                Conn con = new Conn();
    ...
    public static void main(String[] args) {
        new AddStudent();  // Create and show the frame
    }
}
